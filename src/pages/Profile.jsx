...
const Profile = () => {
  ...
  return (
    <div
      className="min-h-screen font-poppins text-gray-800 relative overflow-x-hidden"
      style={{ backgroundColor: "rgba(245,245,245,0.75)", paddingBottom: "4rem" }}
    >
      {/* Header */}
      <motion.div
      ...
      >
        <h1 className="text-5xl md:text-6xl font-extrabold text-gray-900 drop-shadow-lg">
          My Profile <span style={{ color: "inherit" }}>🥳</span>
        </h1>
        <p className="mt-3 text-lg md:text-xl text-gray-600">
          Manage your account, orders & feedback in style <span style={{ color: "inherit" }}>✨</span>
        </p>
      </motion.div>
      ...
      {/* Bottom Section */}
      <div className="mt-16 grid grid-cols-1 md:grid-cols-2 gap-8 px-4 lg:px-16">
        {/* Top Customers */}
        <motion.div
          ...
          {loadingTopUsers ? (
            ...
            <ul className="space-y-3">
              {topUsers.map((user, index) => (
                <li
                  key={index}
                  className={`flex justify-between ${index === 0 ? "text-yellow-500 font-bold" : ""}`}
                >
        ...
        >
          ...
          <textarea
            ...
            className="w-full p-4 border border-gray-300 rounded-xl focus:ring-2 focus:ring-orange-400 focus:outline-none mb-4"
          ></textarea>
          <motion.button
            onClick={handleFeedbackSubmit}
            className="px-6 py-2 bg-gradient-to-r from-orange-400 via-pink-400 to-purple-500 text-white rounded-xl font-semibold shadow-md"
            variants={buttonVariants}
            whileHover="hover"
          >
            Submit Feedback
        ...
        {showModal && selectedProduct && (
          ...
          >
            ...
            >
              ...
              <p className="text-gray-700">{selectedProduct.productId?.desc || ""}</p>
            </motion.div>
          </motion.div>
        )}
      </AnimatePresence>
