...
export default function CodeVerificationAuthPage() {
  ...
  const handleSubmit = async () => {
    if (!number || code.length !== 4) {
      alert("Enter a valid 4-digit code", "error");
      return;
    }

    setLoading(true);
    try {
      const res = await axios.post("https://nepcart-backend.onrender.com/api/auth/verify", {
        number,
        code: code
      });
      ...
      alert(err.response?.data?.msg || "Verification failed", "error");
      setCode("");
    } finally {
      setLoading(false);
    }
      ...
      <div className="relative z-10 flex flex-col md:flex-row items-center justify-center h-full overflow-hidden">
        ...
        <div className="w-full md:w-1/2 text-white px-6 mb-10 md:mb-0 overflow-hidden">
          ...
          <p className="text-[hsl(218,81%,85%)] text-sm">
            One of the best ecommerce platform where you can buy custom printed
            tshirts and also <code>You can upload any photo and print it in tshirt</code> in
            affordable price!! Fast and secured delivery with order tracking and
            easy communication between customer and seller. Delivery all over
            province 1.
          </p>
        </div>
          ...
          <div className="flex justify-center gap-3 mb-6">
            {Array(4)
              ...
              .map((_, i) => (
                <div
                  key={i}
                  className={`w-12 h-12 border rounded-lg flex items-center justify-center text-lg font-semibold ${
                    code[i]
                      ? "bg-white text-black"
